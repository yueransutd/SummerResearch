...
def evaluating(model, datas, best_F):
    ...
    return best_F, new_F, save, feats
...
for epoch in range(1, 10001):
    for i, index in enumerate(np.random.permutation(len(train_data))):
        ...
        if count % (eval_every) == 0 and count > (eval_every * 20) or \
            ...
            model.train(False)
            best_train_F, new_train_F, _, feats = evaluating(model, test_train_data, best_train_F)
            best_dev_F, new_dev_F, save, feats = evaluating(model, dev_data, best_dev_F)
            if save:
                torch.save(model, model_name)
            best_test_F, new_test_F, _, feats = evaluating(model, test_data, best_test_F)
            sys.stdout.flush()
